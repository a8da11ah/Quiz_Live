...
import { startSession, launchSession } from '../lib/api.js'
...
export default function SessionDetail() {
  ...
  // Connect to WS as host once session is in lobby+
  ...
  if (!session) return <div className="min-h-screen bg-gray-950 flex items-center justify-center text-red-400">Session not found</div>
      ...
      <Modal open={forceModal} onClose={() => setForceModal(false)} title="Unmarked answers">
        <div className="flex flex-col gap-4">
          <p className="text-gray-300 text-sm">
            There are <span className="text-yellow-400 font-bold">{pendingMarks}</span> open-text answer{pendingMarks > 1 ? 's' : ''} that haven't been marked yet.
